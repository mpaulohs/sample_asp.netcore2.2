...
using Dapper;
using demo.UI.Site.ControllersApi;
...
namespace demo.UI.Site.Repository
{
    public interface IExportXLS
    {
        void Create(string filePath);
        void Create2(string filePath);
        string Value();
        object GetBlogList();
        Task<Blog2> GetBlogByID(int id);
    }
    public class ExportXLS : IExportXLS
    {
        private readonly IConfiguration _config;

        public ExportXLS(IConfiguration config)
        {
            _config = config;
        }
        ...
        private class TestDisplayNameAttr
        {
            [Display(Name = "A")]
            public String Name { get; set; }
            [Display(Name = "B")]
        ...
        public void Create2(string filePath)
        {
            ...
            var list = new List<TestDisplayNameAttr>();
            list.Add(new TestDisplayNameAttr() { Name = "John", Age = 30, House = "On Elm St." });
            list.Add(new TestDisplayNameAttr() { Name = "Mary", Age = 15, House = "On Main St." });
            list.Add(new TestDisplayNameAttr() { Name = "Luis", Age = 21, House = "On 23rd St." });
            list.Add(new TestDisplayNameAttr() { Name = "Henry", Age = 45, House = "On 5th Ave." });

            ws.FirstCell().InsertTable(list);
            wb.SaveAs(filePath);
        }
        ...
        public string Value()
        {
            ...
            string conString = "User Id=florida;Password=florida; Data Source=localhost:1521/orcl;";
            ...
            cmd.CommandText = "SELECT DESCRIPTION FROM BLOG WHERE ID= 2";
        ...
        public async Task<Blog2> GetBlogByID(int id)
        {
            using (IDbConnection conn =  Connection)
            {
                string sQuery = "SELECT b.Id, b.description FROM blog b where b.ID = @ID";
                // conn.Open();
                var result = await conn.QueryAsync<Blog2>(sQuery, new { ID = id });
        ...
        public IDbConnection Connection
        {
            get
            {
                return new OracleConnection(_config.GetConnectionString("BaseIndicadores"));
            }
        }
    }
}
